 ...
 * アイコンだけの操作と未選択肢は副次色、単独で意味を担う文字付き操作は前景色で置く。
 * 選択状態だけ薄い塗りを許可し、主副の階層は持たない。
 */
const action = cva(
  [
  ...
  ],
  {
    variants: {
      shape: {
        ...
        boxed:
          'rounded-[var(--radius-control)] border border-[var(--color-border-subtle)] px-[var(--spacing-inline)] text-[var(--color-fg-primary)] hover:border-[var(--color-fg-primary)]',
        tab: 'rounded-none border-b border-transparent px-[var(--spacing-inline)] aria-selected:border-b-[var(--color-fg-primary)]',
        // 画面の現在地は、タブの背面を移動する共通のカプセルで示す
        viewTab: 'relative z-1 rounded-[var(--radius-capsule)] px-[var(--spacing-inline)]',
        // 値の選択は、操作要素の形を保った薄い塗りで示す
        segment:
        ...
        bare: 'min-w-[var(--size-hit)] rounded-[var(--radius-control)] text-[var(--color-fg-primary)]',
      },
      /*
